<!DOCTYPE html>
<html>
	<head>
		<title>WebOne</title>
		<link rel="stylesheet" href="/css/base.css">
		<script src="/js/jquery-1.7.2.min.js" type="text/javascript"></script>
	</head>
	<body class="welcome">
		<div class="header">
			<div class="header-cont">
				<form>
					<a href="#" class="right">加入收藏</a>
					<span class="header_divide"></span>
					<a href="#" class="right">设为首页</a>
					<span class="header_divide"></span>
					<input class="right header_login mr20 pointer" type="submit" value="登陆"/>
					<input class="right header_input" type="password" value="密码"/>
					<input class="right header_input" type="text" value="用户名"/>
					<span class="right mr10">管理员登陆:</span>
					<span>欢迎浏览沙枣花网络文化艺术平台</span>
					<span class="ml10">2012年3月11日</span>
					<span class="ml5">星期日</span>
				</form>
			</div>
			<div class="header_cor"></div>
			<div class="header_bg"></div>
		</div>
		<div class="welcome1"></div>
		<div class="welcome2"><a class="welcome_enter" href="#">点击进入</a></div>
		<div class="welcome3"></div>
		<div class="welcome4">
			<p class="c_green">版权所有2012-2020兰州车务段技术信息科</p>
			<p class="c_999">copyright   2012  lanzhou  tie  wu  duan ji shu xin xi ke</p>
		</div>
		<div class="welcome_flash"></div>
	</body>
</html>